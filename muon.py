        ...
        for group in self.param_groups:
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    g = p.grad
                    if g is None:
                        # No gradient on this rank for this parameter: contribute zeros but still
                        # participate in the collective so other ranks do not deadlock.
                        g = torch.zeros_like(update_buffer_views[self.rank])
                    else:
                        state = self.state[p]
                        if "momentum_buffer" not in state:
                            state["momentum_buffer"] = torch.zeros_like(g)
                    ...
                    # This rank has no parameter in this slice: still participate with zeros.
                    g = torch.zeros_like(update_buffer_views[self.rank])
            ...
            # Only update if a collective has actually been launched
