...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        self.z_dim = z_dim
        self.projector = build_mlp(hidden_size, projector_dim, self.z_dim)
    ...
    def forward(
        ...
        for i in range(self.num_f):
            ...
            if enc_idx < len(enc_depths) and layer_idx == enc_depths[enc_idx]:
                ...
                z_enc = self.projector(x_enc.reshape(-1, D_enc)).reshape(B_enc, -1, self.z_dim)
                zs.append(z_enc)
        ...
        for i in range(self.num_f, self.num_f + self.num_g):
            ...
            if enc_idx < len(enc_depths) and layer_idx == enc_depths[enc_idx]:
                ...
                z_mid = self.projector(x_mid.reshape(-1, D_mid)).reshape(B_mid, -1, self.z_dim)
                zs.append(z_mid)
        ...
        for i in range(self.num_f + self.num_g, self.depth):
            ...
            if enc_idx < len(enc_depths) and layer_idx == enc_depths[enc_idx]:
                ...
                z_dec = self.projector(x_dec.reshape(-1, D_dec)).reshape(B_dec, -1, self.z_dim)
                zs.append(z_dec)
                proj_ids_keeps.append(None)
                enc_idx += 1
