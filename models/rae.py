...
class RAE(nn.Module):
    def __init__(self, 
        ...
        pretrained_decoder_path: Optional[str] = "https://huggingface.co/nyu-visionx/RAE-collections/resolve/main/decoders/dinov2/wReg_base/ViTXL_n08/model.pt",
        # ---- noising, reshaping and normalization-----
        noise_tau: float = 0.8,
        normalization_stat_path: Optional[str] = "https://huggingface.co/nyu-visionx/RAE-collections/resolve/main/stats/dinov2/wReg_base/imagenet1k/stat.pt",
        eps: float = 1e-5,
    ):
        super().__init__()
        ...
        decoder_config = AutoConfig.for_model("vit_mae", **decoder_config)
        ...
        if pretrained_decoder_path is not None:
            ...
            # Download https://huggingface.co/nyu-visionx/RAE-collections/resolve/main/decoders/dinov2/wReg_base/ViTXL_n08/model.pt to cache_dir
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)
            vae_path = os.path.join(cache_dir, "rae-vitxl-n08.pt")
            if not os.path.exists(vae_path):
                import requests
                url = "https://huggingface.co/nyu-visionx/RAE-collections/resolve/main/decoders/dinov2/wReg_base/ViTXL_n08/model.pt"
                r = requests.get(url)
                with open(vae_path, "wb") as f:
                    f.write(r.content)
                print(f"Downloaded {vae_path}")

            print(f"Loading pretrained decoder from {vae_path}")
        ...
        if normalization_stat_path is not None:
            ...
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)
            stats_path = os.path.join(cache_dir, "rae-stat.pt")
            if not os.path.exists(stats_path):
                import requests
                url = "https://huggingface.co/nyu-visionx/RAE-collections/resolve/main/stats/dinov2/wReg_base/imagenet1k/stat.pt"
                r = requests.get(url)
